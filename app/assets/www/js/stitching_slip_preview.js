...
async function loadSlipData(orderId) {
    try {
        ...
        document.getElementById('ss-garment-type').textContent = `${o.clothing_type || 'Custom Item'} (x${o.quantity})`;
        ...
        if (o.measurement_profile && Object.keys(o.measurement_profile.values).length > 0) {
            ...
            section.innerHTML = `
                <h3 class="font-headline-md text-headline-md text-primary flex items-center gap-2 border-b border-outline-variant/50 pb-2">
                    ...
                    ${o.measurement_profile.template_type} Measurements
                </h3>
            `;
            ...
            const values = o.measurement_profile.values;
            for (const key of Object.keys(values)) {
                const item = document.createElement('div');
                item.className = "flex flex-col items-center justify-center p-4 bg-surface rounded-lg border border-outline-variant shadow-sm relative overflow-hidden";
                item.innerHTML = `
